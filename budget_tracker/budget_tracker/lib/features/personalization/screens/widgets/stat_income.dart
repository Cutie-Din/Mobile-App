...
class StatIncome extends StatefulWidget {
  final int maNguoiDung;

  const StatIncome({super.key, required this.maNguoiDung});

  final Color leftBarColor = AppColors.primary; // Keep left bar color

  @override
  State<StatefulWidget> createState() => _StatIncomeState();
}

class _StatIncomeState extends State<StatIncome> {
  ...
  void loadExpenseData() {
    ...
    final thuTransactions = transactionsBox.values
        .where((transaction) =>
            transaction.ma_nguoi_dung == widget.maNguoiDung &&
            transaction.category.split('-').first.trim() == 'Thu')
        .toList();
    ...
    for (var transaction in thuTransactions) {
  ...
  Map<String, double> calculateCategoryPercentages() {
    ...
    // Total expenses
    double totalExpenses = thuTransactions.fold(
      0,
