// Flutter & Third-party packages
export 'dart:async';
export 'dart:convert';
export 'package:flutter/foundation.dart';
export 'package:flutter/material.dart';
export 'package:flutter/services.dart';
export 'package:flutter_dotenv/flutter_dotenv.dart';
export 'package:flutter_svg/flutter_svg.dart';
export 'package:fl_chart/fl_chart.dart';
export 'package:font_awesome_flutter/font_awesome_flutter.dart';
export 'package:freezed_annotation/freezed_annotation.dart';
export 'package:gap/gap.dart';
export 'package:get/get.dart' hide formData;
export 'package:google_fonts/google_fonts.dart';
export 'package:image_picker/image_picker.dart';
export 'package:json_annotation/json_annotation.dart';
export 'package:logger/logger.dart';
export 'package:pinput/pinput.dart';
export 'package:pretty_dio_logger/pretty_dio_logger.dart';
export 'package:shared_preferences/shared_preferences.dart';
export 'package:stylish_bottom_bar/stylish_bottom_bar.dart';

// Configuration
export 'package:credit_hub_new/config/app_config.dart';

// Constants
export 'package:credit_hub_new/src/constant/app_colors.dart';
export 'package:credit_hub_new/src/constant/app_constants.dart';
export 'package:credit_hub_new/src/constant/app_images.dart';

// Components
export 'package:credit_hub_new/src/components/bottomsheet/app_bottom_sheet.dart';
export 'package:credit_hub_new/src/components/button/app_button.dart';
export 'package:credit_hub_new/src/components/dialog/app_dialog.dart';
export 'package:credit_hub_new/src/components/loading/app_loading.dart';
export 'package:credit_hub_new/src/components/menu/app_bottom_menu.dart';
export 'package:credit_hub_new/src/components/text/app_text_field.dart';

// Base
export 'package:credit_hub_new/src/data/_base/base_barrel.dart';

// Models
export 'package:credit_hub_new/src/data/model/model_barrel.dart';

// Repositories
export 'package:credit_hub_new/src/data/repository/repository_barrel.dart';

// Service
export 'package:credit_hub_new/src/data/service/service_barrel.dart';

// Shared Utilities
export 'package:credit_hub_new/src/shared/app_manager.dart';
export 'package:credit_hub_new/src/shared/app_route.dart';

// Themes
export 'package:credit_hub_new/theme/app_theme.dart';

// Utilities
export 'package:credit_hub_new/src/utils/app_logger.dart';

// Authentication Screens
export 'package:credit_hub_new/src/ui/auth/change_pass/change_password_screen.dart';
export 'package:credit_hub_new/src/ui/auth/forgot_password/forgot_password_screen.dart';
export 'package:credit_hub_new/src/ui/auth/otp/otp_screen.dart';
export 'package:credit_hub_new/src/ui/auth/sign_in/sign_in_screen.dart';

// Main Screens
export 'package:credit_hub_new/src/ui/main/add/add_screen.dart';
export 'package:credit_hub_new/src/ui/main/dashboard/dashboard_screen.dart';
export 'package:credit_hub_new/src/ui/main/dashboard/widgets/app_chart.dart';
export 'package:credit_hub_new/src/ui/main/history/history_detail/history_detail_screen.dart';
export 'package:credit_hub_new/src/ui/main/history/history_screen/history_screen.dart';
export 'package:credit_hub_new/src/ui/main/notification/notification_screen.dart';

// Account Management Screens
export 'package:credit_hub_new/src/ui/main/account/account_add/account_add_screen.dart';
export 'package:credit_hub_new/src/ui/main/account/account_details/account_details_screen.dart';
export 'package:credit_hub_new/src/ui/main/account/account_edit/account_edit_screen.dart';
export 'package:credit_hub_new/src/ui/main/account/account_list/account_list_screen.dart';
export 'package:credit_hub_new/src/ui/main/account/account_screen/account_screen.dart';

// Other Screens
export 'package:credit_hub_new/src/ui/splash/splash_screen.dart';
